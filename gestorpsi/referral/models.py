...
class ReferralManager(models.Manager):
    def charged(self):
        return super(ReferralManager, self).\
            get_query_set().filter(referraldischarge__isnull=True)

    def discharged(self):
        return super(ReferralManager, self).\
            get_query_set().filter(referraldischarge__isnull=False)
...
class Queue(models.Model):
    ...
    priority = models.CharField(_('Priority'), max_length=2, blank=True,
                                choices=QUEUE_PRIORITY, default='03')
...
class ReferralInRangeManager(models.Manager):
    ...
    def all(self, organization, datetime_start=None, datetime_end=None,
            service=None):
        r = Referral.objects.filter(
            service__organization=organization,
            date__gte=datetime_start, date__lt=datetime_end) \
            .exclude(
                referraldischarge__reason=REFERRAL_DISCHARGE_REASON_CANCELED)
...
class Referral(Event):
    ...
    def add_occurrences(self, start_time, end_time, room, device, annotation,
                        is_online, disable_check_busy=False, reserve=False,
                        **rrule_params):
        ...
        if 'count' not in rrule_params and 'until' not in rrule_params:
            ...
            # try: # this try function is used to book a group.
                # verify if the occurrence inserted is part from the same group
                # occurrence = ScheduleOccurrence.objects.filter(
                    # start_time=ev, end_time=ev + delta,
                    # room=Room.objects.get(pk=room))[0]
                # if occurrence.scheduleoccurrence.event.referral.group and
                    # occurrence.scheduleoccurrence.event.
                        #referral.group == self.group:
            ...
            if not is_busy or disable_check_busy:
                o = ScheduleOccurrence.objects.create(event=self,
            ...
            for ev in rrule.rrule(dtstart=start_time, **rrule_params):
                # adding_to_existing_group = False
                # try: # this try function is used to book a group. verify if
                    # the occurrence inserted is part from the same group
                    # occurrence = ScheduleOccurrence.objects.filter(
                        # start_time=ev, end_time=ev + delta,
                        # room=Room.objects.get(pk=room))[0]
                    # if occurrence.scheduleoccurrence.event.referral.group
                        # and occurrence.scheduleoccurrence.event.
                            # referral.group == self.group:
                        #adding_to_existing_group = True
                #except:
                    #adding_to_existing_group = False
        ...
        for p in self.professional.all():
            if p.is_busy(start_time, end_time):
                error_message.append(_(
                    'Professional %s is busy in this range') % p)
    ...
    def _service_name_html(self):
        return u"<div class='service_name_html' style='background-\
            color:#%s'>&nbsp;</div> %s #%s" % (self.service.color,
                                               self.service.name, self.seq)
    service_name_html = property(_service_name_html)

    def _service_name_html_inline(self):
        return u"%s #%s <div class='service_name_html_inline' style=\
            'background-color:#%s'>&nbsp;</div>" % (self.service.name,
                                                    self.seq,
                                                    self.service.color)
    ...
    def revision(self):
        return reversion.get_for_object(self).\
            ...
            latest('revision__date_created').revision
    ...
    def past_occurrences(self):
        ...
        return self.occurrence_set.filter(start_time__lt=datetime.now()).\
            ...
            exclude(Q(scheduleoccurrence__is_online=True) & Q(
                end_time__gt=datetime.now())).reverse()
    ...
    def confirmed_occurrences(self):
        return self.occurrence_set.filter(
            scheduleoccurrence__occurrenceconfirmation__isnull=False)

    def unconfirmed_occurrences(self):
        return self.occurrence_set.filter(
            scheduleoccurrence__occurrenceconfirmation__isnull=True)

    def confirmed_arrive_ontime(self):
        return self.occurrence_set.filter(
            scheduleoccurrence__occurrenceconfirmation__isnull=False,
            scheduleoccurrence__occurrenceconfirmation__presence=1)

    def confirmed_arrive_late(self):
        return self.occurrence_set.filter(
            scheduleoccurrence__occurrenceconfirmation__isnull=False,
            scheduleoccurrence__occurrenceconfirmation__presence=2)

    def confirmed_not_arrived(self):
        return self.occurrence_set.filter(
            scheduleoccurrence__occurrenceconfirmation__isnull=False,
            scheduleoccurrence__occurrenceconfirmation__presence=3)

    def confirmed_unmarked(self):
        return self.occurrence_set.filter(
            scheduleoccurrence__occurrenceconfirmation__isnull=False,
            scheduleoccurrence__occurrenceconfirmation__presence=4)

    def confirmed_rescheduled(self):
        return self.occurrence_set.filter(
            scheduleoccurrence__occurrenceconfirmation__isnull=False,
            scheduleoccurrence__occurrenceconfirmation__presence=5)
    ...
    def online_and_active_occurrences(self):
        ...
        '''
        return self.occurrence_set.filter(
            start_time__lt=datetime.now()).filter(
            Q(scheduleoccurrence__is_online=True) & Q(
                end_time__gt=datetime.now())).reverse()
    ...
    def professional_list(self):
        a = []
        for p in self.professional.all():
            if(hasattr(p.professionalIdentification, "profession") and p.
                    professionalIdentification.profession):
                a.append(u'%s (%s)' % (p, p.
                                       professionalIdentification.profession))
            else:
                a.append(u'%s' % (p))
    ...
    def occurrences_without_session(self):
        return self.occurrences().exclude(session__isnull=False).\
            order_by('-start_time')
    ...
    def occurrences_without_diagnosis(self):
        return self.occurrences().exclude(diagnosis__isnull=False).\
            order_by('-start_time')
...
class ReferralDischarge(models.Model):
    ...
    date = models.DateTimeField(auto_now_add=True)
    was_discussed_with_client = models.\
        BooleanField(_('Was Discussed With Client'), default=False)
    # reason = models.CharField(_('Discharge Reason'),
    # max_length=2, blank=True, null=True, choices=REFERRAL_DISCHARGE_TYPE)
    reason = models.ForeignKey('ReferralDischargeReason',
                               verbose_name=('Discharge Reason'),
                               blank=True, null=True)
    details = models.TextField(_('Discharge Details'), blank=True)
    status = models.CharField(_('Status'), max_length=2, blank=True,
                              null=True, choices=REFERRAL_DISCHARGE_STATUS)
...
class ReferralDischargeReason(models.Model):
    ...
    color = models.CharField(_('Color'), max_length=6, null=True,
                             help_text=_('Color in HEX Format. Ex: 662393'))
...
class ReferralExternal(models.Model):
    ...
    def __unicode__(self):
        if self.professional:
            return u'%s - %s (%s)' % (self.date.strftime('%d/%m/%Y'),
                                      self.organization, self.professional)
